...
### 7. 使用动态最小候选集合而不是 Top K 打分

Prompt 要求 Agent 按可疑程度从高到低输出能够解释反馈的最小候选集合：Action 为 0～5 个，Event 总数最多 15 个；后续候选缺少独立证据时立即停止。后端不计算分数，只按模型顺序执行硬上限，并删除不归属于已选 Action 的 Event。
