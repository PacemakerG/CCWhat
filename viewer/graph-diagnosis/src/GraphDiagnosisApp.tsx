...
function GraphNavigator({
  ...
  const detailX = Math.max(4, Math.min(width - 48, activeX - 20));
  ...
  return <section className="gd-navigator" aria-label={t.overview}>
    ...
    <svg viewBox={`0 0 ${width} 94`} role="img" aria-label="Workflow navigator">
      ...
      {view === 'action' && activeIndex >= 0 && <>
        <line className="gd-navigator-detail-link" x1={activeX} y1="31" x2={activeX} y2="55" />
        <rect className="gd-navigator-viewport" x={detailX} y="55" width="40" height="26" rx="4" />
        <text className="gd-navigator-detail-label" x={detailX + 20} y="71" textAnchor="middle">···</text>
      </>}
    </svg>
